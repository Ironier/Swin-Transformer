...
class UnNamedBlock(nn.Module):
        ...
        def __init__(self,depth=2,feature_nums=3,img_size=256,gvi_num=4,patch_size=4,embed_dim=96,
            ...
            for i in range(depth):
                layer=GVIAttentionBlock(embed_dim=embed_dim,qkv_bias=qkv_bias,num_head=feature_nums*(2**(depth-i)),
                                        ...
                                        norm_layer=norm_layer)
                self.layers.append(layer)
                layer=nn.ConvTranspose2d(in_channels=feature_nums*2**(depth-i),out_channels=feature_nums*2**(depth-i-1),kernel_size=2,padding=0,stride=2)
                self.layers.append(layer)
                layer=norm_layer(feature_nums*2**(depth-i-1))
                self.layers.append(layer)
        ...
        def forward(self,x,feature):
            ...
            gvis=torch.clamp(x1/x2,min=1e-4,max=1-self.eps)
...
class Decoder(nn.Module):
        def __init__(self,img_size=224,num_classes=15,patch_size=4,depth_nums=[ 2, 4, 8], gvi_nums=[1,2,3],feature_nums=[ 16, 4, 2],
            ...
            self.conv2d=nn.Conv2d(3,self.num_classes,kernel_size=3,padding=1,stride=1)
        ...
        def forward(self,x,feature):
            ...
            cnt=0
            for i in range(self.num_layers):
                x=self.layers[2*i](x,feature)+x
                x=self.layers[2*i+1](x)
                res[:,:,:,:,cnt]=x
                cnt+=1
            output=self.conv3d(res.transpose(1,4)).squeeze()
            output=self.conv2d(output.transpose(1,2)).transpose(1,3)
        ...
        def forward_test(self,x,feature):
            ...
            for i in range(self.num_layers):
                ...
                res[:,:,:,:,cnt]=x
                cnt+=1
            output2=torch.rand((x.shape[0],x.shape[1],x.shape[2],self.num_classes,self.num_layers)).to(device)
            cnt=0
            for i in range(self.num_layers):
                temp=res[:,:,:,:,cnt]
                output2[:,:,:,:,cnt]=self.norm(self.conv2d(temp.transpose(1,3)).transpose(1,3))
                cnt+=1
            output=self.conv3d(res.transpose(1,4)).squeeze()
            output=self.conv2d(output.transpose(1,2)).transpose(1,3)
            output=self.norm(output)
